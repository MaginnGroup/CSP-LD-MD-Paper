...
deltaf_alch = -5.05/4.184/(kb*350.0)
...
for state in range(nstates):
    ...
    data = pd.read_csv(f'../../cubic/{int(temp[state])}/output.out', sep=" ")
    data['U_pot'] = data['PotEng']
    data['PV'] = 1.0*data['Volume']*conv*Navogadro
    data.drop(index=range(5000), inplace=True)
    data.index = np.arange(0, len(data))
    ...
    H_b[state] = np.mean(data['Enthalpy'])/162.0
    a = (data['U_pot'] - data['E_mol'])
    b = (data['U_pot'] - data['E_mol'] + data['PV'])
    cps[state] = (np.mean(a*b) - np.mean(a)*np.mean(b))/162.0/(temp[state])**2/kb +  (np.mean(data['PV']*b) - np.mean(data['PV'])*np.mean(b))/162.0/(temp[state])**2/kb
...
reweighting_sol['f'] = reweighting_sol['f']/162.0
results_liq['f'] = results_liq['f']/162.0
results_sol['f'] = results_sol['f']/162.0
...
params = linear_fit(temp[6:10], deltaHp[6:10])

fig = plt.figure(figsize=(3.0, 2.0),  dpi=300)
plt.ylabel(r'$\Delta H_{s \rightarrow l}$ (kJ/mol)')
plt.xlabel(r'$T$ (K)')
plt.plot(temp, deltaHp*4.184, 'ko', label='')
plt.plot(temp, (np.array(temp) * params[0] + params[1])*4.184, color='red')
