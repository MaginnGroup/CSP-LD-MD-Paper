...
deltaf_alch = -6.70/4.184/(kb*350.0)
...
for state in range(nstates):
    ...
    a = (data['U_pot'] - data['E_mol'])
    b = (data['U_pot'] - data['E_mol'] + data['PV'])
    cps[state] = (np.mean(a*b) - np.mean(a)*np.mean(b))/200.0/(temp[state])**2/kb +  (np.mean(data['PV']*b) - np.mean(data['PV'])*np.mean(b))/200.0/(temp[state])**2/kb
...
for point in range(nstates):
    ...
    dev_deltaGp[point] = np.sqrt( (results_sol['df'][point])**2 + (results_liq['df'][point])**2 + \
                     (results_sol['df'][0])**2 + (results_liq['df'][0])**2 + \
