...
deltaf_alch = -2.40/4.184/(kb*350.0)
...
for state in range(nstates):
    ...
    data = pd.read_csv(f'../../../1_NPT_equilibration/index-13/Outputs/{int(temp[state])}_output.out', sep=" ")
    data['U_pot'] = data['PotEng']
    data['PV'] = 1.0*data['Volume']*conv*Navogadro
    data.drop(index=range(5000), inplace=True)
    data.index = np.arange(0, len(data))
    ...
    a = (data['U_pot'] - data['E_mol'])
    b = (data['U_pot'] - data['E_mol'] + data['PV'])
    cps[state] = (np.mean(a*b) - np.mean(a)*np.mean(b))/180.0/(temp[state])**2/kb +  (np.mean(data['PV']*b) - np.mean(data['PV'])*np.mean(b))/180.0/(temp[state])**2/kb
...
params = linear_fit(temp[4:7], deltaHp[4:7])
