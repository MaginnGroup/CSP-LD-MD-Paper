...
deltaf_alch = -1.66/4.184/(kb*350.0)
...
for state in range(nstates):
    ...
    data = pd.read_csv(f'../../../1_NPT_equilibration/index-3/Outputs/{int(temp[state])}_output.out', sep=" ")
    data['U_pot'] = data['PotEng']
    data['PV'] = 1.0*data['Volume']*conv*Navogadro
    data.drop(index=range(5000), inplace=True)
    data.index = np.arange(0, len(data))
    ...
    H_b[state] = np.mean(data['Enthalpy'])/200.0
...
reweighting_sol['Hp'] = reweighting_sol['Hp']/200.0
reweighting_liq['dHp'] = reweighting_liq['dHp']/162.0
reweighting_sol['dHp'] = reweighting_sol['dHp']/200.0
results_liq['f'] = results_liq['f']/162.0
results_sol['f'] = results_sol['f']/200.0
results_liq['df'] = results_liq['df']/162.0
results_sol['df'] = results_sol['df']/200.0
...
plt.xlabel(r'$T$ (K)')
plt.plot(temp, lala*4.184, 'ko', label='')
plt.plot(temp, deltaHp*4.184, 'k-', label='')
...
plt.xlabel(r'$T$ (K)')
plt.plot(temp, -lala2*temp*4.184, 'ko', label='')
plt.plot(temp, -deltaSp*temp*4.184, 'k-', label='')
